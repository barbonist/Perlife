...
void alwaysPumpPressLoopArt(unsigned char pmpId, unsigned char *PidFirstTime)
{
    ...
    if (SpeedCostanteArt((int)actualSpeed_Art) &&
        (actualSpeed_Art <= MAX_ART_RPM) && (sensor_UFLOW[ARTERIOUS_AIR_SENSOR].Average_Flow_Val < (float)parameterWordSetFromGUI[PAR_SET_MAX_FLOW_PERFUSION].value)
		)
    {
...
//   deltaSpeed_Art = kpForTuning * errPress;
	...
	else if (sensor_UFLOW[ARTERIOUS_AIR_SENSOR].Average_Flow_Val == 0 && pressSample0_Art < Target_PID_ART)
	{
...
//   deltaSpeed_Ven = kpForTuning_Ven * errPress;
   deltaSpeed_Ven = ((parKITC_Ven * errPress) -
	...
	else if((sensor_UFLOW[1].Average_Flow_Val == 0.0) && (pressSample0_Ven < Target_PID_VEN))
	{
